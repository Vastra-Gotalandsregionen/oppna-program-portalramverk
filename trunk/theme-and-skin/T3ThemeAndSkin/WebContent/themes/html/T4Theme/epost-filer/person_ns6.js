...
wps_userLeftFuncs=new Object();var pa_addtostlist_window=null;var pa_addtostlist_window_opener=null;var pa_stlinksPrivateGroups=new Array();var linkId=null;var defaultDynamicPersonTagURL=null;var registeredInstantMessagingEvents=false;var personjsSTLinksWatchNames_true="";var personjsSTLinksWatchNames_false="";var persontag_isLoggedIn=false;var persontag_awarenessversion=""; // indicates whether we are using classic ST or SIP IM
...
{
var menuDataTagObject=awarenessController.getMenuDataTagObject(counter);var menuData=menuDataTagObject.getMenuData();if (menuData == null)
{
...
userIdType="LDAPDN";}var params="value=" + URLEncoder(userId) + "&valueType=" + userIdType;params+="&getMenuData=TRUE";if (typeof contextArray != "undefined" && contextArray != null && contextArray != "") {
...
url+="?" + params;};var maindata=getDataFromServer(url);index=maindata.indexOf("<dynamicpersontagdata>");var start=index + "<dynamicpersontagdata>".length;maindata=maindata.substr(start, maindata.indexOf("</dynamicpersontagdata>")-start);menuData=maindata;menuDataTagObject.setMenuData(menuData);}eval(menuData);var IMLogin=menuDataTagObject.getIMLogin();var linkDisplayName=menuDataTagObject.getDisplayName();var businessCard=new BusinessCard();var menuDisplayName=displayName;if (displayName == null || displayName == '') {
menuDisplayName=linkDisplayName;}businessCard.setVariables(menuDisplayName,cardData);var menu=buildMenu(IMLogin,menuList,businessCard, target, isSubMenu);return menu;};function invokeDynamicPersonTag (userId, userIdType, displayName, IMLogin, contextArray) {
...
window.open(kmapURL);};function portalShowAddToContactsUI(name){
pa_addtostlist_window_opener=window;linkId=name;STLinksGetPrivateGroups();};var awarenessController=new AwarenessController();function BusinessCard(){
...
switch (status) {
case "0":        //offline
statusMessage=this.offlineMsg.replace(/%s/gi, this.getDisplayNameForUser(userId));break;case "32":        //active
...
statusMessage=this.offlineMsg.replace(/%s/gi, this.getDisplayNameForUser(userId));};}person.setStatus(status);person.setStatusMessage(statusMessage);person.update();};function updatePersonLink(userId) {
var key;userId=userId.toLowerCase();for (key in personArray) {
if (key.toLowerCase() == userId) {
userId=key;}};var person=personArray[userId];if (typeof person == "undefined" || person == null)
...
downImage=this.createDownImage(counter);menuLink=this.createMenuLink(userId,userIdType,displayName,IMLogin, counter);endTag="</a>";}var output=menuLink + statusImage + displayName + downImage + endTag;return output;};function setDynamicPTagServletBaseURL(junk) {
...
{
retStr=str.substring(0, indexComma) + "\\"  + str.substring(indexComma);}}return retStr;};function PAaddtoSTList(group)
{
STLinksAddToContactList(escapeSlashComma(linkId), group);};function STLinkClicked(person, personLinkText, status, event)
{
linkId=person;if (typeof STLinksCreateIM == "function")
STLinksCreateIM(escapeSlashComma(linkId));};function STLinksPrivateGroupsReceived(groups)
{
...
{
if (pa_addtostlist_window_opener == window)
{
pa_stlinksPrivateGroups=new Array();if (groups != null && groups.length > 0)
{
pa_stlinksPrivateGroups=groups.split(";");}pa_addtostlist_window=window.open(pmi_stlist, "pa_addtostlist", "resizable=yes,width=400,height=500,toolbar=no,status=no,menubar=no", true);}pa_addtostlist_window_opener=null;};function STLinksAddToContactListFailed(reason)
{
if (pa_addtostlist_window != null &&
typeof pa_addtostlist_window.getErrorString == "function")
{
alert(pa_addtostlist_window.getErrorString() + ": " + reason);}};function getStlinksPrivateGroups()
{
return pa_stlinksPrivateGroups;};function getAddToSTListPerson()
{
return linkId;};var InputHandler_pabData=null;var InputHandler_handlerArray=new Array();function InputHandler_getServerPabData()
{
