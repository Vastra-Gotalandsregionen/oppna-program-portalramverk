...
if (!e) {
e=window.event;}if (e.keyCode == 17 && !isCtrlDepressed()) {
document._CtrlKeyDepressed=true;for (var i=0; i < window.frames.length; i++) {
window.frames[i].document._CtrlKeyDepressed=true;};setTimeout("document._CtrlKeyDepressed=false;", 500);}if (document._DNDBroker.oldOnKeyDown != null) {
document._DNDBroker.oldOnKeyDown();}};function keyup(e){
if (!e) {
e=window.event;}if (e.keyCode == 17) {
...
document.body.style.cursor="";}};function DNDCopyToParent(dndSrc){
var parentDoc=window.frames["top"].document;var srcDiv=document.getElementById(dndSrc.getId());var newDiv=parentDoc.createElement("div");parentDoc.body.appendChild(newDiv);dndSrc.id=dndSrc.getId() + "_frame";window.frames["top"].document._DNDBroker.registerSource(dndSrc);newDiv.setAttribute("id", dndSrc.getId());newDiv.className=dndSrc.getCssClassName();newDiv.fromChildFrame="true";newDiv.innerHTML=srcDiv.innerHTML;newDiv.mouseOffsetX=srcDiv.mouseOffsetX;newDiv.mouseOffsetY=srcDiv.mouseOffsetY;newDiv.style.visibility="hidden";parentDoc._DNDBroker.childFrame=window.frames["self"];};DNDBroker.prototype=new Object();DNDBroker.prototype.constructor=DNDBroker;DNDBroker.superclass=null;function DNDBroker() {
this.DNDSourceArray=new Array(0);this.DNDTargetArray=new Array(0);this.selectedSource=null;this.awareTargetsArray=null;this.multiselectSupport=new DNDMultiselectSupport();this.dndUtil=new DNDUtil();this.dynamicFunctions=new Array();};DNDBroker.prototype.registerSource=function(dndSrc){
this.DNDSourceArray.push(dndSrc);};DNDBroker.prototype.registerTarget=function(dndTgt){
this.DNDTargetArray.push(dndTgt);};DNDBroker.prototype.beginDrag=function(dndSrc_id, x, y){
...
elem.mouseOffsetX=this.originalPageX - util.findOffsetX(elem);elem.mouseOffsetY=this.originalPageY - util.findOffsetY(elem);}};if (window.frames["top"] != window.frames["self"]) {
...
newSSIdArray[i]=selectedSourceIdArray[i] + "_frame";};window.frames["top"].document._DNDBroker.multiselectSupport.selected_src_ids=newSSIdArray;window.frames["top"].document._DNDBroker.multiselectSupport.selected_src_property_intersection=this.multiselectSupport.getProperties();}window.frames["top"].document._DNDBroker.beginDrag(dndSrc_id + "_frame", x, y);}this.dragCloneSupport=new DNDDragCloneSupport();this.dragCloneSupport.beginDrag(selectedSourceIdArray, this.selectedSource.getDragClone());this.doBeginDragBSPCreation();if (this.selectedSource.getCssClassName() != null) {
...
this.doTargetSubmit();}}if (!doSubmit) {
...
this.multiselectSupport=new DNDMultiselectSupport();}}this.selectedSource=null;this.awareTargetsArray=null;this.formParamNames=null;this.formParamValues=null;this._frameOriginalX=null;this._frameOriginalY=null;setDNDNormalCursor();};DNDBroker.prototype.move=function(x, y){
...
this.id=id;this.action=action;this.properties=dndPropertyArray;this.activeCssClassName=activeCssClassName;this.awareCssClassName=awareCssClassName;this.supportsMultiselect=supportsMultiselect;this.multiselectDelimiter=multiselectDelimiter;};DNDAction.prototype.getId=function(){
